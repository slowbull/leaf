...
def main():
    ...
    client_model.setup_gamma(args.gamma)
    #if args.mom == 0:
    #    server_model = ServerModel(ClientModel(*model_params))
    #else:
    #    server_model = ServerModel_mom(ClientModel(*model_params))
    server_model = ServerModel_mom(ClientModel(*model_params), eta=args.eta, beta=args.mom)
    ...
    print('Rounds Train_acc_avg  p_10  p_90  Train_loss_avg  p_10  p_90  Test_acc_avg  p_10 p_90  Test_loss_avg  p_10 p_90', flush=True)
    ...
    server.model.setup(all_num_samples)
...
def parse_args():
    ...
    parser.add_argument('-eta',
                    help='learning rate for global optimizers;',
                    type=float,
                    default=0.01,
                    required=False)
    parser.add_argument('-gamma',
                    help='discount factor for global optimizers;',
                    type=float,
                    default=1.0,
                    required=False)
...
def print_metrics(metrics, weights, rounds):
    ...
    print('{} '.format(rounds), end='', flush=True)
    for metric in metric_names:
        ...
        print('{:.4f}   {:.4f}   {:.4f} '.format(\
                 np.average(ordered_metric, weights=ordered_weights),
                 np.percentile(ordered_metric, 10),
                 np.percentile(ordered_metric, 90)), end='', flush=True)
